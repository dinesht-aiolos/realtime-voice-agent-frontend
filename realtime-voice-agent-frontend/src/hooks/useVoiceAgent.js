...
const WS_CLOSE_MESSAGES = {
  1001: "Server went away.",
  1006: "Backend not reachable — is your Python server running?",
  1008: "Auth failed — check API keys",
  1011: "Server error — check backend logs.",
};

export function useVoiceAgent(provider = "deepgram") {
  const [status, setStatus] = useState(STATUS.IDLE);
  const [messages, setMessages] = useState([]);
  const [error, setError] = useState(null);
  ...
  const enqueueAudio = useCallback((int16) => {
    ...
    src.onended = () => {
      scheduledSrcsRef.current = scheduledSrcsRef.current.filter(
        (s) => s !== src
      );
    };
  }, []);
  ...
  startMicrophone(wsRef.current);
        ...
        case "AgentAudioDone":
          setTimeout(() => updateStatus(STATUS.READY), 300);
          break;

        case "Error":
          setError(msg.description || "Error");
          updateStatus(STATUS.ERROR);
          break;
  ...
  }, [provider, handleMessage, startMicrophone, updateStatus]);
